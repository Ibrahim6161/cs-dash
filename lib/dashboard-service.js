...
class DashboardService {
  ...
  createSteps() {
    ...
    const csstonksScript = path.join(this.rootDir, "scrapers", "csstonks", "scrape-csstonks.js");
    const caseDetailsScript = path.join(this.rootDir, "scrapers", "csstonks", "scrape-case-details.js");
    const caseTimeseriesScript = path.join(this.rootDir, "scrapers", "csstonks", "scrape-case-timeseries.js");
    const steamCasesScript = path.join(this.rootDir, "scrapers", "steam", "scrape-steam-cases.cjs");
    const steamSkinsScript = path.join(this.rootDir, "scrapers", "steam", "scrape-steam-skins.cjs");

    if (fs.existsSync(csstonksScript)) {
      steps.push({
        id: "csstonks",
        label: "CSStonks cases",
        optional: true,
        command: node,
        args: [csstonksScript, path.join(this.rootDir, "cases.json")],
      });
    }

    if (fs.existsSync(caseDetailsScript)) {
      steps.push({
        id: "case-details",
        label: "Case details",
        optional: true,
        ...
        args: [
          caseDetailsScript,
          "--cases",
          path.join(this.rootDir, "cases.json"),
          "--out",
          path.join(this.rootDir, "case_details.json"),
          "--shots",
          path.join(this.rootDir, "shots"),
          "--headless",
        ],
      });
    }

    if (fs.existsSync(caseTimeseriesScript)) {
      steps.push({
        id: "case-timeseries",
        label: "Case timeseries",
        optional: true,
        command: node,
        args: [caseTimeseriesScript],
      });
    }
