...
function buildDashboard(root, config, extra = {}) {
  ...
  const skinbaron = extractMarketplaceCollection(readJson(pricempirePath));
  const timeseriesRaw = readJson(timeseriesPath);
  const externalMarkets = skinbaron;
  ...
  const skinbaronCovered = Object.values(marketMap).filter((record) =>
    Array.isArray(record?.prices) && record.prices.some((price) => price.providerKey === "skinbaron")
  ).length;
  const universeCount = normalizedUniverseKeys.size;
  const missingPricingProviders = [];
  if (marketCovered === 0) missingPricingProviders.push("SkinBaron");
  ...
  const list = universe.map((base) => {
    ...
    const skinbaronEur = pickProviderPriceByKeys(marketplacePrices, ["skinbaron"]);
  ...
  for (const item of list) {
    ...
    if (item.marketplaces.prices.length >= 4) {
      item.thesis.push(`${item.marketplaces.prices.length} external SkinBaron quote${item.marketplaces.prices.length === 1 ? "" : "s"} are available for confirmation.`);
    }
    if (item.metrics.externalSpreadPct != null && item.metrics.externalSpreadPct <= 6) {
      item.thesis.push("External pricing is tightly aligned, which adds confidence to the current read.");
    }
  ...
  if (!externalMarkets.timestamp) diagnostics.push("SkinBaron pricing is missing, so external market confirmation is unavailable.");
  if (externalMarkets.timestamp && marketCovered === 0) diagnostics.push("SkinBaron pricing exists, but it contains zero matched prices for the current case universe.");
  ...
  return {
    ...
    refreshHealth: {
      ...
      universeCount,
      pricingCoverage: {
        skinbaron: marketCovered,
        providers: externalMarkets.providerCount,
        skinbaronDirect: skinbaronCovered,
      },
    },
    diagnostics,
    cases: list,
