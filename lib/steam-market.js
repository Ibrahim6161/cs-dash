...
function toIsoDate(label) {
  const match = cleanText(label).match(/^([A-Za-z]{3})\s+(\d{1,2})\s+(\d{4})/);
...
function summariseHistory(points) {
  ...
  return {
    ...
    avgVolume30d: points30.length
      ...
      : null,
    volatility30dPct: avg30 && variance30 != null
...
function extractHistoryScript(html) {
  ...
  return parseHistoryArray(match ? match[1] : null);
}
...
function parseOrderbook(payload) {
  ...
  const spreadPct = lowestSellEur && highestBuyEur
...
async function fetchSteamMarket(name, options = {}) {
  ...
  const [overview, listingResponse] = await Promise.all([
    fetchSteamOverview(name, currency),
    requestText(buildListingUrl(name), { accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8" }),
  ]);
  ...
  const history = extractHistoryScript(listingResponse.body);
  ...
  if ((includeOrderbook || overview.failed) && itemNameId) {
    const orderbookUrl = `https://steamcommunity.com/market/itemordershistogram?country=${country}&language=${language}&currency=${currency}&item_nameid=${itemNameId}&two_factor=0`;
    const orderbookResponse = await requestText(orderbookUrl, { accept: "application/json,text/plain,*/*" });
