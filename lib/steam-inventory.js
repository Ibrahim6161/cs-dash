...
function pushAttempt(debug, stage, response) {
  ...
  debug.attempts.push({
    ...
    statusCode: response?.statusCode || 0,
    payloadKeys: response?.payload && typeof response.payload === "object" ? Object.keys(response.payload).slice(0, 20) : [],
...
function groupInventoryItems(raw, valuationLookup) {
  ...
  for (const asset of raw.assets) {
    ...
    const existing = grouped.get(key) || {
      ...
      iconUrl: description.icon_url ? `https://community.fastly.steamstatic.com/economy/image/${description.icon_url}/96fx96f` : null,
  ...
  const items = [...grouped.values()].map((item) => {
    const valuation = valuationLookup(item);
    ...
    return {
      ...
      listingUrl: valuation?.listingUrl || (item.marketable ? `https://steamcommunity.com/market/listings/730/${encodeURIComponent(item.marketHashName)}` : null),
      dashboardKey: valuation?.dashboardKey || null,
      dashboardItem: valuation?.dashboardItem || null,
    };
  });
...
async function enrichWithLivePrices(items, cache, limit = 24) {
  ...
  for (const item of targets) {
    ...
    try {
      ...
      // Some items intermittently fail on the lightweight overview endpoint
      // while still having usable listing/history data on the main market page.
      if (unitPriceEur == null) {
        try {
          const fullMarket = await fetchSteamMarket(item.marketHashName || item.name, { includeOrderbook: false });
