...
function normalizePct(value) {
  ...
  // Convert only real ratio-style values like -0.2865 -> -28.65
  // Do NOT convert already-normal percentage values like -0.88
  if (Math.abs(num) < 1) {
...
function extractDetails(raw, shotsDir) {
  ...
  for (const record of records) {
    ...
    map[normalizeName(name)] = {
	  ...
	  delta1m: normalizePct(pageCards.delta1m?.percent ?? record.delta1m ?? pageCards.delta1m),
	  delta6m: normalizePct(pageCards.delta6m?.percent ?? record.delta6m ?? pageCards.delta6m),
      delta12m: normalizePct(pageCards.delta12m?.percent ?? record.delta12m ?? pageCards.delta12m),
...
function computeSeriesStats(points) {
  ...
  const avg = (values) =>
    values.length ? values.reduce((sum, value) => sum + value, 0) / Math.max(1, values.length) : null;
...
function buildDashboard(root, config, extra = {}) {
  ...
  for (const item of items) {
    ...
    const supplyTrend = item.metrics.momentum12m != null
      ? clamp01((20 - item.metrics.momentum12m) / 40)
      : null;
