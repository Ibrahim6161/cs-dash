...
async function resolvePublicSteamProfile(rawQuery) {
  ...
  if (!candidateUrls.length) {
    const error = new Error(
      "Invalid Steam profile input. Use a Steam profile URL, vanity URL, or SteamID64."
    );
  ...
  for (const url of candidateUrls) {
    ...
    const profileUrl =
      extractTag(response.body, "profileurl") ||
      `https://steamcommunity.com/profiles/${steamId}/`;
    const avatar =
      extractTag(response.body, "avatarFull") ||
      extractTag(response.body, "avatarMedium") ||
      extractTag(response.body, "avatarIcon");
  ...
  error.debug = {
    ...
    lastStatusCode: lastResponse?.statusCode || null,
    lastUrl: lastResponse?.url || null,
    bodyPreview: String(lastResponse?.body || "").slice(0, 800),
  };
  throw error;
}
